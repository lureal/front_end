...
$('#submit').click(() => {
    ...
    if (type === '1') {
        ...
        // 营业执照
        if (
            $('#business > div').attr('data-url') === undefined ||
            $('#business > div').attr('data-url') === ''
        ) {
            modaler.tip('请上传营业执照');
            return;
        } else {
            fields['businessLicense'] = $('#business > div').attr('data-url');
        }

        // 一般纳税人证明
        if (
            $('#tax-certification > div').attr('data-url') === undefined ||
            $('#tax-certification > div').attr('data-url') === ''
        ) {
            modaler.tip('请上传一般纳税人证明');
            return;
        } else {
            fields['taxCertification'] = $('#tax-certification > div').attr('data-url');
        }
    ...
    requester.get('/external/finance/applyInvoice.do', fields).then(data => {
        if (data.data === true) {
            modaler.confirm({
                ...
                evt () {
                    $('#modal-confirm').modal();
                    location.href = `/#proj_name#/html/finance/invoice.html?cid=${urler().cid}`;
                },
                evtLabel: '确定'
            });
        } else {
