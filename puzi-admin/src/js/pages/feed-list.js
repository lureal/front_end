 ...
 */
var feedList = function(ajax, modal, time) {

	requestList(1, undefined);

	// 取消订阅
	$('#feed-list').on('click', '#feed-list-delete', function() {
	...
	// 向前翻页
	$('#feed-list').on('click', '.previous', function() {
	...
	// 指定页数分页
	$('#feed-list').on('click', '.page', function() {
		var pagecount = Number($(this).attr('data-pagecount')); // 分页总数
		var pagenow = Number($(this).attr('data-pagenow')); // 当前分页
	...
	// 指定页数翻页
	$('#feed-list').on('click', '.next', function() {
		var pagecount = Number($(this).attr('data-pagecount')); // 分页总数
		var pagenow = Number($(this).attr('data-pagenow')); // 当前分页
		var $this = $(this);
	...
	function requestList(page, $targetBtn) {
		...
		// 获取列表数据
		ajax.get({
			url: '/admin/subscriber/list.do',
			param: { page: page },
			cb: function(data) {

				// 格式化开始时间和结束时间
				for(var i = 0; i < data.data.subscribers.length; i++) {
					var val = data.data.subscribers[i];
					val.posttime_str = time.unixToTime(val.posttime);
				}

				// 渲染界面
				var tpl = $('#feed-list-tpl').html();
				$('#feed-list-wrap').html(_.template(tpl)(data));
